...
- (void)animationDidStop:(NSString *)animationID finished:(NSNumber *)finished context:(void *)theContext {
	...
	if ([animationID isEqual:@"FadeIn"]) {
		...
		popoverVisible = YES;
        if ([delegate respondsToSelector:@selector(popoverControllerDidAppear:)]) {
            [delegate popoverControllerDidAppear:self];
        }
...
- (void)presentPopoverFromRect:(CGRect)rect 
    ...
    UIView *aView = contentViewController.view;
    aView = nil;
...
- (void)dismissPopoverAnimated:(BOOL)animated userInitiated:(BOOL)userInitiated {
	if (self.view) {
		...
		if (animated) {
			...
			[UIView beginAnimations:@"FadeOut" context:(void*)[NSNumber numberWithBool:userInitiated]];
			[UIView setAnimationDelegate:self];
			[UIView setAnimationDidStopSelector:@selector(animationDidStop:finished:context:)];
