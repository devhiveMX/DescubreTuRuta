...
@implementation NSString (NSString_IsNumber)

- (BOOL)isNumberOfMaxLength:(NSInteger)length {
    NSString *numberRegex = [NSString stringWithFormat:@"[0-9]{0,%d}", length];
    
    NSPredicate *emailTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", numberRegex];
    
    BOOL matches = [emailTest evaluateWithObject:self];
    return matches;
}

- (BOOL)isNumberOfMinLength:(NSInteger)minLen andMaxLength:(NSInteger)maxLen {
    NSString *numberRegex = [NSString stringWithFormat:@"[0-9]{%d,%d}", minLen, maxLen];
...
- (NSString*)stringByConvertingToISoLatin {
    NSData *asciiData = [self dataUsingEncoding:NSUTF8StringEncoding allowLossyConversion:YES];
    return [[NSString alloc] initWithData:asciiData encoding:NSISOLatin1StringEncoding];
}
